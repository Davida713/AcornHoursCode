#This is the version locally approved after the project was completed.
...
engine = create_engine('postgresql://catalog:catalog@localhost/acornhours')
Base.metadata.bind = engine

DBSession = sessionmaker(bind=engine)
session = DBSession()

#Home page for showing types of events.
#Will adjust header based on if user is logged in.
...
#User can create a new type. Only logged in users can create new record. 
#Owner ID is passed through login_session so no need to input ID.
...
def newType():
    ...
    if request.method == 'POST':
        addtype= ServiceType(name=request.form['name'], 
...
#delete function is limited to the user called owner who created the record.
@app.route('/ServiceTypes/<int:servicetype_id>/delete/', 
    methods=['GET', 'POST'])
def deleteType(servicetype_id):
    dtype = session.query(ServiceType).filter_by(id=servicetype_id).one()
    if ('username' not in login_session or 
            dtype.owner != login_session['user_id']):
        ...
        if request.method == 'POST':
            ...
            return render_template('deletetype.html', 
                dtype=dtype, username= login_session['username'])

'''
Edit function can only be accedded by the owner. 
if not logged in or not the owner you are redirected. 
Not logged in users are not shown the edit button and can not access from URL.
'''
...
def editType(servicetype_id):
    ...
    if ('username' not in login_session or 
            etype.owner != login_session['user_id']):
        ...
        if request.method == 'POST':
            ...
            return render_template('edittype.html', 
                    etype=etype, username= login_session['username'])
#public inormation. This page changed based on status in the template pages logic. 
#no need for seperate private and public templates this way.
...
#Create new even is only for logged in users. 
...
def newEvent(servicetype_id):
    if 'username' not in login_session:
        ...
        if request.method == 'POST':
            ...
            aevent= Event(name=request.form['name'], date=fixdate, time=fixtime, 
                    description=request.form['description'], 
                    address=request.form['address'], 
                    type_id=type.id, owner= login_session['user_id'])
            session.add(aevent)
            session.commit()
            ...
            return render_template('newevent.html', 
                    type=type, username= login_session['username'])

#Delete event can omly be accessed by owner

@app.route('/ServiceTypes/<int:servicetype_id>/events/<int:event_id>/delete/', 
    methods=['GET', 'POST'])
def deleteEvent(servicetype_id, event_id):
    dtype = session.query(ServiceType).filter_by(id=servicetype_id).one()
    devent= session.query(Event).filter_by(id=event_id).one()
    if ('username' not in login_session or 
            devent.owner != login_session['user_id']):
        ...
        if request.method == 'POST':
            ...
            return render_template('deleteevent.html', devent=devent, 
                    dtype= dtype, username= login_session['username'])

#Edit event can only be acced by the owner. 
@app.route('/ServiceTypes/<int:servicetype_id>/events/<int:event_id>/edit/', 
    methods=['GET', 'POST'])
def editEvent(servicetype_id, event_id):
    etype = session.query(ServiceType).filter_by(id=servicetype_id).one()
    eevent= session.query(Event).filter_by(id=event_id).one()
    if ('username' not in login_session or 
            eevent.owner != login_session['user_id']):
        ...
        if request.method == 'POST':
            ...
            return render_template('editevent.html', etype=etype, 
                    eevent=eevent, username= login_session['username'])

#This is the lowest level in the data heiarchy. It time and date are shown here. 
...
#Allows access to the log in with google APT
...
def showLogin():
    ...
    if 'username' not in login_session:
        ...
        return render_template('login.html', 
                STATE=state, username= login_session['username'])

#This was largely taken from the code used int eh OAUTH cource. 
...
'''
There is an issue of the token not being revoked,
if you do not clear the cashe before disconnection.
Open a new incognito window to fix this. Should not be a problem for a new user.
'''
...
#These helper functions support login and user creation
...
#uncomment the clear session method when testing to clear the session.
'''
...
'''
#JSON functions give every element from the DB
